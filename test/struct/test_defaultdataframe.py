...
from raven.struct.dataframe.bytecolumn import ByteColumn
from raven.struct.dataframe.shortcolumn import ShortColumn
from raven.struct.dataframe.intcolumn import IntColumn
from raven.struct.dataframe.longcolumn import LongColumn
from raven.struct.dataframe.floatcolumn import FloatColumn
from raven.struct.dataframe.doublecolumn import DoubleColumn
from raven.struct.dataframe.stringcolumn import StringColumn
from raven.struct.dataframe.charcolumn import CharColumn
...
class TestDefaultDataFrame(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.df = DefaultDataFrame(
            DataFrame.ByteColumn(column_names[0], [10,20,30,40,50]),
            DataFrame.ShortColumn(column_names[1], [11,21,31,41,51]),
            DataFrame.IntColumn(column_names[2], [12,22,32,42,52]),
            DataFrame.LongColumn(column_names[3], [13,23,33,43,53]),
            DataFrame.StringColumn(column_names[4], ["10","20","30","40","50"]),
            DataFrame.CharColumn(column_names[5], ["a","b","c","d","e"]),
            DataFrame.FloatColumn(column_names[6], [10.1,20.2,30.3,40.4,50.5]),
            DataFrame.DoubleColumn(column_names[7], [11.1,21.2,31.3,41.4,51.5]),
            DataFrame.BooleanColumn(column_names[8], [True,False,True,False,True]),
            DataFrame.BinaryColumn(column_names[9], [bytearray.fromhex("05"),
                                                     bytearray.fromhex("0060"),
                                                     bytearray.fromhex("000070"),
                                                     bytearray.fromhex("00000080"),
                                                     bytearray.fromhex("0000000090")])
            )

        self.toBeSorted = DefaultDataFrame(
            DataFrame.ByteColumn(column_names[0], [4,2,1,5,3]),
            DataFrame.ShortColumn(column_names[1], [4,2,1,5,3]),
            DataFrame.IntColumn(column_names[2], [4,2,1,5,3]),
            DataFrame.LongColumn(column_names[3], [4,2,1,5,3]),
            DataFrame.StringColumn(column_names[4], ["4","2","1","5","3"]),
            DataFrame.CharColumn(column_names[5], ["d","b","a","e","c"]),
            DataFrame.FloatColumn(column_names[6], [4.0,2.0,1.0,5.0,3.0]),
            DataFrame.DoubleColumn(column_names[7], [4.0,2.0,1.0,5.0,3.0]),
            DataFrame.BooleanColumn(column_names[8], [True,False,True,False,True]),
            DataFrame.BinaryColumn(column_names[9], [bytearray.fromhex("00000080"),
                                                     bytearray.fromhex("0060"),
        ...
        self.column_types = [ByteColumn.TYPE_CODE,
    ...
    def test_constructor_no_args(self):
        test = DefaultDataFrame()
        self.assertTrue(test.is_empty(), "DefaultDataFrame should be empty")
        self.assertTrue(test.rows() == 0, "DefaultDataFrame row count should be 0")
        self.assertTrue(test.columns() == 0, "DefaultDataFrame column count should be 0")
        self.assertFalse(test.has_column_names(),
                         "DefaultDataFrame should not have column names set")

        self.assertTrue(isinstance(test, DefaultDataFrame), "Is not DefaultDataFrame type")

    def test_constructor_with_columns(self):
        test = DefaultDataFrame(
            DataFrame.IntColumn(values=[1,2,3]),
            DataFrame.StringColumn(values=["1","2","3"]),
            DataFrame.ByteColumn(values=[1,2,3]))

        self.assertFalse(test.is_empty(), "DefaultDataFrame should not be empty")
        self.assertTrue(test.rows() == 3, "DefaultDataFrame row count should be 3")
        self.assertTrue(test.columns() == 3, "DefaultDataFrame column count should be 3")
        self.assertFalse(test.has_column_names(),
                         "DefaultDataFrame should not have column names set")
    ...
    def test_constructor_with_labeled_columns(self):
        ...
        test = DefaultDataFrame(
            DataFrame.IntColumn(names[0], [1,2,3]),
            DataFrame.StringColumn(names[1], ["1","2","3"]),
            DataFrame.ByteColumn(names[2], [1,2,3]))

        self.assertFalse(test.is_empty(), "DefaultDataFrame should not be empty")
        self.assertTrue(test.rows() == 3, "DefaultDataFrame row count should be 3")
        self.assertTrue(test.columns() == 3, "DefaultDataFrame column count should be 3")
        self.assertTrue(test.has_column_names(), "DefaultDataFrame should have column names set")
        self.assertEqual(names,
                         test.get_column_names(),
                         "DefaultDataFrame column names do not match")
    ...
    def test_get_short_by_index(self):
        self.assertTrue(self.df.get_short(1, 3) == 41, "Short at index 3 should be 41")

    def test_get_short_by_name(self):
        self.assertTrue(self.df.get_short("shortCol", 3) == 41, "Short at index 3 should be 41")

    def test_get_int_by_index(self):
        self.assertTrue(self.df.get_int(2, 1) == 22, "Int at index 1 should be 22")

    def test_get_int_by_name(self):
        self.assertTrue(self.df.get_int("intCol", 1) == 22, "Int at index 1 should be 22")
    ...
    def test_get_float_by_index(self):
        self.assertAlmostEqual(
            self.df.get_float(6, 1), 20.2, places=5, msg="Float at index 1 should be 20.2")

    def test_get_float_by_name(self):
        self.assertAlmostEqual(
            self.df.get_float("floatCol", 1), 20.2, places=5,
            msg="Float at index 1 should be 20.2")
    ...
    def test_get_double_by_name(self):
        self.assertAlmostEqual(
            self.df.get_double("doubleCol", 4), 51.5, places=5,
            msg="Double at index 4 should be 51.5")

    def test_get_boolean_by_index(self):
        self.assertFalse(
            self.df.get_boolean(8, 1), "Boolean at index 1 should be false")

    def test_get_boolean_by_name(self):
        self.assertFalse(
            self.df.get_boolean("booleanCol", 1), "Boolean at index 1 should be false")

    def test_get_binary_by_index(self):
        self.assertTrue(
            self.df.get_binary(9, 1) == bytearray.fromhex("0060"),
            "Binary at index 1 should be 0060")

    def test_get_binary_by_name(self):
        self.assertTrue(
            self.df.get_binary("binaryCol", 1) == bytearray.fromhex("0060"),
            "Binary at index 1 should be 0060")
    ...
    def test_set_char_by_index(self):
        ...
        self.assertTrue(self.df.get_char(5, 2) == "T",
                        "Char at index 2 should be set to \'T\'")

    def test_set_char_by_name(self):
        self.df.set_char("charCol", 2, "T")
        self.assertTrue(self.df.get_char("charCol", 2) == "T",
                        "Char at index 2 should be set to \'T\'")

    def test_set_float_by_index(self):
        self.df.set_float(6, 1, 11.2)
        self.assertAlmostEqual(self.df.get_float(6, 1), 11.2, places=5,
                               msg="Float at index 1 should be 11.2")

    def test_set_float_by_name(self):
        self.df.set_float("floatCol", 1, 11.2)
        self.assertAlmostEqual(self.df.get_float("floatCol", 1), 11.2, places=5,
                               msg="Float at index 1 should be 11.2")

    def test_set_double_by_index(self):
        self.df.set_double(7, 4, 11.3)
        self.assertAlmostEqual(self.df.get_double(7, 4), 11.3, places=5,
                               msg="Double at index 4 should be 11.3")

    def test_set_double_by_name(self):
        self.df.set_double("doubleCol", 4, 11.3)
        self.assertAlmostEqual(self.df.get_double("doubleCol", 4), 11.3, places=5,
                               msg="Double at index 4 should be 11.3")
    ...
    def test_set_binary_by_index(self):
        ...
        self.assertTrue(val == bytearray.fromhex("abcd"),
                        "Binary at index 1 should be set to 0xabcd")
    ...
    def test_get_column_names(self):
        ...
        self.assertSequenceEqual(
            self.column_names,
            names, "Column names do not match array content")

    def test_get_column_name(self):
        self.assertEqual(
            "longCol", self.df.get_column_name(3),
            "Column name for column at index 3 does not equal \"longCol\"")

        self.assertEqual(
            "longCol", self.df.get_column(3).get_name(),
            "Column name for column at index 3 does not equal \"longCol\"")

    def test_get_column_index(self):
        self.assertTrue(
            self.df.get_column_index("longCol") == 3,
            "Column \"longCol\" is not at index 3")
    ...
    def test_set_column_names_varargs(self):
        ...
        self.assertSequenceEqual(
            names, self.df.get_column_names(), "Column names do not match set names")
    ...
    def test_remove_column_names(self):
        ...
        self.assertFalse(
            self.df.has_column_names(),
            "Test-DataFrame should not have column names set")
    ...
    def test_get_row(self):
        ...
        self.assertSequenceAlmostEqual(
            (20,21,22,23,"20","b",20.2,21.2,False,bytearray.fromhex("0060")),
    ...
    def test_set_row(self):
        row = (42,42,42,42,"42","A",42.2,42.2,True,bytearray.fromhex("1122"))
        self.df.set_row(1, row)
        self.assertSequenceAlmostEqual(
            self.df.get_row(1), row, "Row does not match set values")

    def test_add_row(self):
        self.df.add_row((42,42,42,42,"42","A",42.2,42.2,True,bytearray.fromhex("1122")))
        self.assertTrue(self.df.rows() == 6, "Row count should be 6")
        row = self.df.get_row(5)
        self.assertSequenceAlmostEqual(
            (42,42,42,42,"42","A",42.2,42.2,True,bytearray.fromhex("1122")),
            row, "Row does not match added values")

    def test_add_row_invalid_char(self):
        self.assertRaises(
            DataFrameException, self.df.add_row, [42,42,42,42,"42","€",42.2,42.2,True])

    def test_insert_row(self):
        self.df.insert_row(2, (42,42,42,42,"42","A",42.2,42.2,True,bytearray.fromhex("1122")))
        self.assertTrue(self.df.rows() == 6, "Row count should be 6")
    ...
    def test_insert_row_zero(self):
        self.df.insert_row(0, (42,42,42,42,"42","A",42.2,42.2,True,bytearray.fromhex("1122")))
        self.assertTrue(self.df.rows() == 6, "Row count should be 6")
    ...
    def test_insert_row_end(self):
        self.df.insert_row(5, (42,42,42,42,"42","A",42.2,42.2,True,bytearray.fromhex("1122")))
        self.assertTrue(self.df.rows() == 6, "Row count should be 6")
        row = self.df.get_row(5)
        self.assertSequenceAlmostEqual(
            (42,42,42,42,"42","A",42.2,42.2,True,bytearray.fromhex("1122")),
    ...
    def test_insert_row_invalid_char(self):
        self.assertRaises(
            DataFrameException, self.df.insert_row, 1, [42,42,42,42,"42","€",42.2,42.2,True])
    ...
    def test_remove_rows(self):
        ...
        row = self.df.get_row(1)
        self.assertSequenceAlmostEqual(
            (40,41,42,43,"40","d",40.4,41.4,False,bytearray.fromhex("00000080")),
    ...
    def test_remove_rows_regex_match(self):
        ...
        self.assertTrue(self.df.rows() == 3, "Row count should be 3")
        removed = self.df.remove_rows(8, "False")
        self.assertTrue(removed == 2, "Remove count should be 2")
    ...
    def test_remove_rows_regex_match_by_name(self):
        ...
        self.assertTrue(removed == 2, "Remove count should be 2")
        self.assertTrue(self.df.rows() == 3, "Row count should be 3")
        removed = self.df.remove_rows("booleanCol", "False")
        self.assertTrue(removed == 2, "Remove count should be 2")
        self.assertTrue(self.df.rows() == 1, "Row count should be 1")
        row = self.df.get_row(0)
        self.assertSequenceAlmostEqual(
    ...
    def test_remove_rows_null_regex_match(self):
        ...
        self.assertTrue(r == 0, "Return value should be zero")
        self.assertTrue(self.df.rows() == 5, "Returned DataFrame should have 5 rows")
        ...
        self.assertTrue(self.df.get_int("intCol", 1) == 22, "Value should be 22")
        self.assertTrue(self.df.get_int("intCol", 2) == 32, "Value should be 32")
        self.assertTrue(self.df.get_int("intCol", 3) == 42, "Value should be 42")
        self.assertTrue(self.df.get_int("intCol", 4) == 52, "Value should be 52")

    def test_add_rows(self):
        df2 = DefaultDataFrame(
            ByteColumn(values=[11,22]),
            ShortColumn(values=[11,22]),
            IntColumn(values=[11,22]),
            LongColumn(values=[11,22]),
            StringColumn(values=["11","22"]),
            CharColumn(values=["A","B"]),
            FloatColumn(values=[11.1,22.2]),
            DoubleColumn(values=[11.1,22.2]),
            BooleanColumn(values=[True,False]),
            BinaryColumn(values=[bytearray.fromhex("11"),bytearray.fromhex("22")]))
        ...
        df2 = DataFrame.convert_to(df2, "NullableDataFrame")
        self.df.add_rows(df2)
    ...
    def test_add_rows_unlabeled_fraction(self):
        ...
        self.assertSequenceAlmostEqual(
            self.df.get_row(5),
            [11,11,11,0,StringColumn.DEFAULT_VALUE,CharColumn.DEFAULT_VALUE,
             0.0,0.0,False,bytearray.fromhex("00")],
            "Rows do not match")

        self.assertSequenceAlmostEqual(
            self.df.get_row(6),
            [22,22,22,0,StringColumn.DEFAULT_VALUE,CharColumn.DEFAULT_VALUE,
             0.0,0.0,False,bytearray.fromhex("00")],
            "Rows do not match")
    ...
    def test_add_column(self):
        col = DataFrame.IntColumn(values=[0,1,2,3,4])
    ...
    def test_remove_column_by_index(self):
        ...
        self.assertTrue(isinstance(self.df.get_column(3), StringColumn),
                        "Column after removal point should be of type StringColumn")

        self.assertTrue(isinstance(self.df.get_column(2), IntColumn),
                        "Column before removal point should be of type IntColumn")
    ...
    def test_remove_column_by_reference_no_removal(self):
        col = FloatColumn("TEST", self.df.rows())
        res = self.df.remove_column(col)
    ...
    def test_insert_column(self):
        ...
        self.assertTrue(isinstance(self.df.get_column(3), IntColumn),
                        "Column after insertion point should be of type IntColumn")

        self.assertTrue(isinstance(self.df.get_column(1), ShortColumn),
                        "Column before insertion point should be of type ShortColumn")

    def test_insert_column_with_name(self):
        col = DataFrame.IntColumn("INT", [0,1,2,3,4])
        self.df.insert_column(2, col)
        self.assertTrue(self.df.columns() == 11, "Column count should be 11")
        self.assertTrue(col is self.df.get_column(2), "Column reference should be the same")
        self.assertTrue(col is self.df.get_column("INT"), "Column reference should be the same")
    ...
    def test_get_column(self):
        ...
        self.assertTrue(isinstance(col, IntColumn),
                        "Column at index 2 should be of type IntColumn")

    def test_get_column_by_name(self):
        col = self.df.get_column("stringCol")
        self.assertTrue(isinstance(col, StringColumn),
                        "Column \"stringCol\" should be of type StringColumn")
    ...
    def test_get_columns(self):
        ...
        self.assertTrue(isinstance(res, DefaultDataFrame),
    ...
    def test_get_columns_by_name(self):
        ...
        self.assertTrue(res.rows() == 5, "DataFrame should have 5 rows")
        self.assertTrue(
            isinstance(res, DefaultDataFrame), "DataFrame should be a DefaultDataFrame")
    ...
    def test_set_column_by_name(self):
        col = IntColumn("shouldBeReplaced", [0,1,2,3,4])
    ...
    def test_index_of_by_name_with_start_point(self):
        i = self.df.index_of("intCol", "42", start_from=2)
        self.assertTrue(i == 3, "Found index should be 3")
    ...
    def test_index_of_all(self):
        ...
        self.assertTrue(len(i) == 4, "Returned array should have length 4")
        truth = [0,1,2,3]
        self.assertSequenceEqual(
    ...
    def test_filter(self):
        ...
        self.assertTrue(isinstance(filtered, DefaultDataFrame),
                        "Returned DataFrame should be of type DefaultDataFrame")

        self.assertTrue(filtered.rows() == 4, "Returned DataFrame should have 4 rows")
        self.assertTrue(filtered.columns() == 10, "Returned DataFrame should have 10 columns")
        self.assertTrue(filtered.get_int("intCol", 1) == 22, "Int value should be 22")
        self.assertSequenceAlmostEqual(
            [10,11,12,13,"10","a",10.1,11.1,True,bytearray.fromhex("05")],
    ...
    def test_drop(self):
        ...
        self.assertTrue(filtered.columns() == 10, "Returned DataFrame should have 10 columns")
        self.assertTrue(filtered.get_int("intCol", 1) == 52, "Invalid value")
        self.assertSequenceAlmostEqual(
            [40,41,42,43,"40","d",40.4,41.4,False,bytearray.fromhex("00000080")],
            filtered.get_row(0),
    ...
    def test_include(self):
        ...
        self.assertTrue(self.df.rows() == 4, "DataFrame should have 4 rows")
        self.assertTrue(self.df.columns() == 10, "DataFrame should have 10 columns")
        self.assertTrue(self.df.get_int("intCol", 1) == 22, "Invalid value")
        self.assertSequenceAlmostEqual(
            [10,11,12,13,"10","a",10.1,11.1,True,bytearray.fromhex("05")],
            self.df.get_row(0),
            "Row does not match expected values")
    ...
    def test_exclude(self):
        ...
        self.assertTrue(self.df.columns() == 10, "DataFrame should have 10 columns")
        self.assertTrue(self.df.get_int("intCol", 1) == 52, "Invalid value")
        self.assertSequenceAlmostEqual(
            [40,41,42,43,"40","d",40.4,41.4,False,bytearray.fromhex("00000080")],
            self.df.get_row(0),
            "Row does not match expected values")
    ...
    def test_replace(self):
        ...
        replaced_booleans = self.df.replace(8, "False", True)
        self.assertTrue(replaced_longs == 3, "Replaced number should be 3")
        self.assertTrue(replaced_strings == 2, "Replaced number should be 2")
        ...
        self.assertTrue(self.df.get_long(3, 1) == 666, "Value does not match replaced value")
        self.assertTrue(self.df.get_long(3, 2) == 666, "Value does not match replaced value")
        self.assertTrue(self.df.get_long(3, 3) == 43, "Value does not match replaced value")
        ...
        self.assertTrue(self.df.get_string(4, 1) == "20", "Value does not match replaced value")
        self.assertTrue(self.df.get_string(4, 2) == "30", "Value does not match replaced value")
        self.assertTrue(self.df.get_string(4, 3) == "TEST", "Value does not match replaced value")
    ...
    def test_replace_by_name(self):
        ...
        self.assertTrue(
            self.df.get_long("longCol", 1) == 666, "Value does not match replaced value")
        self.assertTrue(
            self.df.get_long("longCol", 2) == 666, "Value does not match replaced value")
        self.assertTrue(
            self.df.get_long("longCol", 3) == 43, "Value does not match replaced value")
        self.assertTrue(
        ...
        self.assertTrue(
            self.df.get_string("stringCol", 1) == "20", "Value does not match replaced value")
        self.assertTrue(
            self.df.get_string("stringCol", 2) == "30", "Value does not match replaced value")
        self.assertTrue(
            self.df.get_string("stringCol", 3) == "TEST", "Value does not match replaced value")
        self.assertTrue(
    ...
    def test_replace_by_name_lambda(self):
        ...
        replaced_booleans = self.df.replace("booleanCol", replacement=lambda i, v: False)
        self.assertTrue(replaced_longs == 5, "Replaced number should be 5")
        self.assertTrue(replaced_strings == 5, "Replaced number should be 5")
        self.assertTrue(replaced_booleans == 3, "Replaced number should be 3")
    ...
    def test_replace_by_name_regex_lambda(self):
        ...
        replaced_booleans = self.df.replace("booleanCol", "False", lambda i, v: True)
        self.assertTrue(replaced_longs == 3, "Replaced number should be 3")
        self.assertTrue(replaced_strings == 2, "Replaced number should be 2")
        self.assertTrue(replaced_booleans == 2, "Replaced number should be 2")
    ...
    def test_replace_dataframe(self):
        df2 = DefaultDataFrame(
            IntColumn(values=[44,44,44,44,44]),
            FloatColumn(values=[44.4,44.4,44.4,44.4,44.4]),
            DoubleColumn(values=[44.4,44.4,44.4,44.4,44.4]),
            BooleanColumn(values=[False,True,False,True,False]))
    ...
    def test_factor(self):
        ...
        self.assertTrue(len(map1) == 4, "Factor map should have a size of 4")
        self.assertTrue(len(map2) == 4, "Factor map should have a size of 4")
        self.assertTrue(len(map3) == 2, "Factor map should have a size of 2")
        self.assertTrue(
            self.df.get_column(4).type_code() == IntColumn.TYPE_CODE,
            "Column should be an IntColumn")

        self.assertTrue(
            self.df.get_column(5).type_code() == IntColumn.TYPE_CODE,
            "Column should be an IntColumn")

        self.assertTrue(
            self.df.get_column(8).type_code() == IntColumn.TYPE_CODE,
            "Column should be an IntColumn")
    ...
    def test_factor_by_name(self):
        ...
        self.assertTrue(len(map3) == 2, "Factor map should have a size of 2")
        self.assertTrue(
            self.df.get_column("stringCol").type_code() == IntColumn.TYPE_CODE,
            "Column should be an IntColumn")

        self.assertTrue(
            self.df.get_column("charCol").type_code() == IntColumn.TYPE_CODE,
            "Column should be an IntColumn")

        self.assertTrue(
            self.df.get_column("booleanCol").type_code() == IntColumn.TYPE_CODE,
            "Column should be an IntColumn")
    ...
    def test_factor_numeric_column(self):
        ...
        self.assertTrue(not map1, "Factor map should be empty")
        self.assertTrue(
            self.df.get_column("byteCol").type_code() == ByteColumn.TYPE_CODE,
            "Column should be a ByteColumn")
        self.assertTrue(
            self.df.get_column("shortCol").type_code() == ShortColumn.TYPE_CODE,
            "Column should be a ShortColumn")
        self.assertTrue(
            self.df.get_column("intCol").type_code() == IntColumn.TYPE_CODE,
            "Column should be an IntColumn")
        self.assertTrue(
            self.df.get_column("longCol").type_code() == LongColumn.TYPE_CODE,
            "Column should be a LongColumn")
        self.assertTrue(
            self.df.get_column("floatCol").type_code() == FloatColumn.TYPE_CODE,
            "Column should be a FloatColumn")
        self.assertTrue(
            self.df.get_column("doubleCol").type_code() == DoubleColumn.TYPE_CODE,
            "Column should be a DoubleColumn")
    ...
    def test_replace_fail_with_null(self):
        self.assertRaises(
            DataFrameException, self.df.replace, 3, "(1|2|3)3", lambda v: None)

    def test_replace_by_name_fail_with_null(self):
        self.assertRaises(
            DataFrameException, self.df.replace, "longCol", "(1|2|3)3", lambda v: None)

    def test_replace_lambda_fail_with_null(self):
        self.assertRaises(
            DataFrameException, self.df.replace, 3, None, lambda v: None)

    def test_replace_regex_lambda_fail_with_null(self):
        self.assertRaises(
            DataFrameException, self.df.replace, 3, "(1|2|3)3", lambda v: None)
    ...
    def test_count(self):
        ...
        self.assertTrue(count.rows() == 5, "Count should have 5 rows")
        ...
        self.assertTrue(
            isinstance(count.get_column(0), StringColumn),
            "Value column should be a StringColumn")

        self.assertTrue(
            isinstance(count.get_column(1), IntColumn),
            "Count column should be an IntColumn")

        self.assertTrue(
            isinstance(count.get_column(2), FloatColumn),
            "Rate column should be a FloatColumn")

        self.assertTrue(
            isinstance(count, DefaultDataFrame),
            "Count DataFrame should be a DefaultDataFrame")
        ...
        self.assertTrue(count.sum(1) == self.df.rows(), "Counts sum is incorrect")
        self.assertTrue(
            isinstance(count.get_column(0), BooleanColumn),
            "Value column should be a BooleanColumn")
    ...
    def test_count_regex(self):
        ...
        self.assertTrue(count == 4, "Count should be 4")
    ...
    def test_count_unique(self):
        ...
        self.assertTrue(count == 5, "Unique count should be 5")
    ...
    def test_unique(self):
        ...
        self.assertTrue(len(set1) == 5, "Unique set size should be 5")
        truth_int = {12, 22, 32, 42, 52}
        self.assertTrue(set1 == truth_int, "Sets should be equal")

        set2 = self.df.unique(4)
        self.assertTrue(len(set2) == 5, "Unique set size should be 5")
        truth_string = {"10", "20", "30", "40", "50"}
        self.assertTrue(set2 == truth_string, "Sets should be equal")

        set3 = self.df.unique(8)
        self.assertTrue(len(set3) == 2, "Unique set size should be 2")
        truth_boolean = {True, False}
        self.assertTrue(set3 == truth_boolean, "Sets should be equal")

        self.df.set_char(5, 4, "a")
        set4 = self.df.unique(5)
        self.assertTrue(len(set4) == 4, "Unique set size should be 4")
        truth_char = {"a", "b", "c", "d"}
        self.assertTrue(set4 == truth_char, "Sets should be equal")

        self.df.set_binary(9, 4, bytearray.fromhex("05"))
        set5 = self.df.unique(9)
        self.assertTrue(len(set5) == 4, "Unique set size should be 4")
        truth_binary = {bytes(bytearray.fromhex("05")), bytes(bytearray.fromhex("0060")),
                        bytes(bytearray.fromhex("000070")), bytes(bytearray.fromhex("00000080"))}
    ...
    def test_difference_columns(self):
        ...
        df2 = DefaultDataFrame(
            StringColumn("A", ["bba","bbb","bbc"]),
            FloatColumn("C", [1.1,2.2,3.3]),
            IntColumn("B", [11, 22, 33]),
            CharColumn("D", ["a", "b", "c"]))

        df3 = df1.difference_columns(df2)
        self.assertFalse(df3.is_nullable(), "DataFrame has an invalid type")
    ...
    def test_difference_columns_invalid_arg(self):
        df1 = DataFrame.Nullable(
            DataFrame.NullableStringColumn("A", ["aaa", "aab", "aac"]))
    ...
    def test_difference_rows(self):
        df1 = DefaultDataFrame(
            StringColumn("A", ["aaa", "aab", "aac", "aab"]),
            IntColumn("B", [1, 2, 3, 2]),
            IntColumn("C", [1, 2, 3, 2]))

        df2 = DefaultDataFrame(
            StringColumn("A", ["bba", "aab", "bbc", "aab"]),
            IntColumn("B", [1, 2, 3, 2]),
            IntColumn("C", [1, 2, 3, 2]))
    ...
    def test_difference_rows_unlabeled(self):
        df1 = DefaultDataFrame(
            StringColumn(values=["aaa", "aab", "aac", "aab"]),
            IntColumn(values=[1, 2, 3, 2]),
            IntColumn(values=[1, 2, 3, 2]))

        df2 = DefaultDataFrame(
            StringColumn(values=["bba", "aab", "bbc", "aab"]),
            IntColumn(values=[1, 2, 3, 2]),
            IntColumn(values=[1, 2, 3, 2]))
    ...
    def test_union_rows_unlabeled(self):
        ...
        df2 = DefaultDataFrame(
            StringColumn(values=["bba", "aab", "bbc"]),
    ...
    def test_convert_from_bytecolumn(self):
        self.df.add_column(ByteColumn("data"))
        self.df.replace("data", replacement=lambda i, v: 0 if i % 2 == 0 else 1)
    ...
    def test_convert_from_shortcolumn(self):
        self.df.add_column(ShortColumn("data"))
        self.df.replace("data", replacement=lambda i, v: 0 if i % 2 == 0 else 1)
        for _, code in enumerate(self.column_types):
            df2 = self.df.clone().convert("data", code)
            df2 = df2.convert("data", self.df.get_column("data").type_code())
    ...
    def test_convert_from_intcolumn(self):
        self.df.add_column(IntColumn("data"))
        self.df.replace("data", replacement=lambda i, v: 0 if i % 2 == 0 else 1)
        for _, code in enumerate(self.column_types):
            df2 = self.df.clone().convert("data", code)
            df2 = df2.convert("data", self.df.get_column("data").type_code())
    ...
    def test_convert_from_longcolumn(self):
        self.df.add_column(LongColumn("data"))
        self.df.replace("data", replacement=lambda i, v: 0 if i % 2 == 0 else 1)
        for _, code in enumerate(self.column_types):
            df2 = self.df.clone().convert("data", code)
            df2 = df2.convert("data", self.df.get_column("data").type_code())
    ...
    def test_convert_from_floatcolumn(self):
        self.df.add_column(FloatColumn("data"))
        self.df.replace("data", replacement=lambda i, v: 0.0 if i % 2 == 0 else 1.0)
        for _, code in enumerate(self.column_types):
            df2 = self.df.clone().convert("data", code)
            df2 = df2.convert("data", self.df.get_column("data").type_code())
            self.assertTrue(df2.equals(self.df), "Conversion failure")
    ...
    def test_convert_from_doublecolumn(self):
        self.df.add_column(DoubleColumn("data"))
        self.df.replace("data", replacement=lambda i, v: 0.0 if i % 2 == 0 else 1.0)
        for _, code in enumerate(self.column_types):
            df2 = self.df.clone().convert("data", code)
            df2 = df2.convert("data", self.df.get_column("data").type_code())
    ...
    def test_convert_from_stringcolumn(self):
        self.df.add_column(StringColumn("data"))
        self.df.replace("data", replacement=lambda i, v: "0" if i % 2 == 0 else "1")
        for _, code in enumerate(self.column_types):
            df2 = self.df.clone()
            if code == BinaryColumn.TYPE_CODE:
                df2.replace("data", "0", replacement="00")
                df2.replace("data", "1", replacement="10") # swap to match the first character
            ...
            df2.replace("data", replacement=lambda v: v[0])
            # change correctly from boolean values
            df2.replace("data", "F", replacement="0")
            df2.replace("data", "T", replacement="1")
            self.assertTrue(df2.equals(self.df), "Conversion failure")

    def test_convert_from_charcolumn(self):
        self.df.add_column(CharColumn("data"))
        self.df.replace("data", replacement=lambda i, v: "0" if i % 2 == 0 else "1")
        for _, code in enumerate(self.column_types):
            df2 = self.df.clone().convert("data", code)
            df2 = df2.convert("data", self.df.get_column("data").type_code())
    ...
    def test_convert_from_binarycolumn(self):
        self.df.add_column(BinaryColumn("data"))
        self.df.replace(
            "data",
            replacement=lambda i, v: bytearray.fromhex("00")
                        if i % 2 == 0
                        else bytearray.fromhex("01"))

        df2 = self.df.clone().convert("data", ByteColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        self.df.replace(
            ...
            replacement=lambda i, v: bytearray.fromhex("0000")
                        if i % 2 == 0
                        else bytearray.fromhex("0001"))

        df2 = self.df.clone().convert("data", ShortColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        self.df.replace(
            ...
            replacement=lambda i, v: bytearray.fromhex("00000000")
                        if i % 2 == 0
                        else bytearray.fromhex("00000001"))

        df2 = self.df.clone().convert("data", IntColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        self.df.replace(
            ...
            replacement=lambda i, v: bytearray.fromhex("0000000000000000")
                        if i % 2 == 0
                        else bytearray.fromhex("0000000000000001"))

        df2 = self.df.clone().convert("data", LongColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        self.df.replace(
            ...
            replacement=lambda i, v: bytearray(struct.pack(">f", 0.0))
                        if i % 2 == 0
                        else bytearray(struct.pack(">f", 1.0)))

        df2 = self.df.clone().convert("data", FloatColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        self.df.replace(
            ...
            replacement=lambda i, v: bytearray(struct.pack(">d", 0.0))
                        if i % 2 == 0
                        else bytearray(struct.pack(">d", 1.0)))

        df2 = self.df.clone().convert("data", DoubleColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        self.df.replace(
            ...
            replacement=lambda i, v: bytearray("0".encode("utf-8"))
                        if i % 2 == 0
                        else bytearray("1".encode("utf-8")))

        df2 = self.df.clone().convert("data", StringColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        df2 = self.df.clone().convert("data", CharColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        df2 = self.df.clone().convert("data", BooleanColumn.TYPE_CODE)
        df2 = df2.convert("data", self.df.get_column("data").type_code())
        self.assertTrue(df2.equals(self.df), "Conversion failure")
        ...
        df2 = self.df.clone().convert("data", BinaryColumn.TYPE_CODE)
        self.assertTrue(df2.equals(self.df), "Conversion failure")
    ...
    def test_group_minimum_by(self):
        df1 = DefaultDataFrame(
            DataFrame.CharColumn("A", ["a", "b", "c", "b", "b", "a"]),
            DataFrame.StringColumn("B", ["aaa", "aab", "aac", "aab", "aab", "aaa"]),
            DataFrame.FloatColumn("C", [5.5, 2.2, 3.3, 4.4, 1.1, 6.6]),
            DataFrame.StringColumn("D", ["bba", "bbb", "bbc", "bbb", "bbb", "bba"]),
            DataFrame.IntColumn("E", [5, 2, 3, 4, 1, 6]),
            DataFrame.LongColumn("F", [5, 2, 3, 4, 1, 6]))

        df2 = df1.group_minimum_by("B")
        self.assertFalse(df2.is_nullable(), "DataFrame has an invalid type")
        ...
        df2.sort_by(0)
        df3 = DefaultDataFrame(
            DataFrame.StringColumn("B", ["aaa", "aab", "aac"]),
            DataFrame.FloatColumn("C", [5.5, 1.1, 3.3]),
            DataFrame.IntColumn("E", [5, 1, 3]),
            DataFrame.LongColumn("F", [5, 1, 3]))
    ...
    def test_group_maximum_by(self):
        ...
        df2.sort_by(0)
        df3 = DefaultDataFrame(
            DataFrame.CharColumn("A", ["a", "b", "c"]),
            DataFrame.FloatColumn("C", [6.6, 4.4, 3.3]),
            DataFrame.IntColumn("E", [6, 4, 3]),
            DataFrame.LongColumn("F", [6, 4, 3]))
    ...
    def test_group_average_by(self):
        ...
        df2.round("F", 2)
        df3 = DefaultDataFrame(
            DataFrame.StringColumn("D", ["bba", "bbb", "bbc"]),
            DataFrame.DoubleColumn("C", [6.05, 2.57, 3.3]),
            DataFrame.DoubleColumn("E", [5.5, 2.33, 3.0]),
            DataFrame.DoubleColumn("F", [5.5, 2.33, 3.0]))
    ...
    def test_group_sum_by(self):
        ...
        df3 = DefaultDataFrame(
            DataFrame.CharColumn("A", ["a", "b", "c"]),
            DataFrame.DoubleColumn("C", [12.1, 7.7, 3.3]),
            DataFrame.DoubleColumn("E", [11.0, 7.0, 3.0]),
            DataFrame.DoubleColumn("F", [11.0, 7.0, 3.0]))
    ...
    def test_median(self):
        ...
        self.df.add_row(
            [127, 420, 402, 420,"42", "A", 420.2, 420.2, True, bytearray.fromhex("0042")])

        self.assertTrue(self.df.median(0) == 35.0, "Computed median should be 35")
        self.assertTrue(self.df.median(1) == 36.0, "Computed median should be 36")
        self.assertTrue(self.df.median(2) == 37.0, "Computed median should be 37")
        self.assertTrue(self.df.median(3) == 38.0, "Computed median should be 38")
        self.assertAlmostEqual(
            35.35, self.df.median(6), places=5, msg="Computed median should be 35.35")
        self.assertAlmostEqual(
            36.35, self.df.median(7), places=5, msg="Computed median should be 36.35")
    ...
    def test_median_by_name(self):
        ...
        self.df.add_row(
            [127, 420, 420, 420, "42", "A", 420.2, 420.2, True, bytearray.fromhex("0042")])

        self.assertTrue(self.df.median("byteCol") == 35.0, "Computed median should be 35")
        self.assertTrue(self.df.median("shortCol") == 36.0, "Computed median should be 36")
        self.assertTrue(self.df.median("intCol") == 37.0, "Computed median should be 37")
        self.assertTrue(self.df.median("longCol") == 38.0, "Computed median should be 38")
        self.assertAlmostEqual(
            35.35, self.df.median("floatCol"), places=5, msg="Computed median should be 35.35")
        self.assertAlmostEqual(
            36.35, self.df.median("doubleCol"), places=5, msg="Computed median should be 36.35")

    def test_median_with_nan(self):
        self.df.clear()
        self.assertTrue(
            math.isnan(self.df.median("byteCol")), "Computed median should be NaN")

    def test_sum(self):
        self.assertTrue(self.df.sum(0) == 150.0, "Computed sum should be 150")
        self.assertTrue(self.df.sum(1) == 155.0, "Computed sum should be 155")
        self.assertTrue(self.df.sum(2) == 160.0, "Computed sum should be 160")
        self.assertTrue(self.df.sum(3) == 165.0, "Computed sum should be 165")
        self.assertAlmostEqual(
            151.5, self.df.sum(6), places=5, msg="Computed sum should be 151.5")
        self.assertAlmostEqual(
            156.5, self.df.sum(7), places=5, msg="Computed sum should be 156.5")

    def test_sum_by_name(self):
        self.assertTrue(self.df.sum("byteCol") == 150.0, "Computed sum should be 150")
        self.assertTrue(self.df.sum("shortCol") == 155.0, "Computed sum should be 155")
        self.assertTrue(self.df.sum("intCol") == 160.0, "Computed sum should be 160")
        self.assertTrue(self.df.sum("longCol") == 165.0, "Computed sum should be 165")
        self.assertAlmostEqual(
            151.5, self.df.sum("floatCol"), places=5, msg="Computed sum should be 151.5")
        self.assertAlmostEqual(
            156.5, self.df.sum("doubleCol"), places=5, msg="Computed sum should be 156.5")
    ...
    def test_minimum_rank_large(self):
        ...
        self.assertTrue(res1.rows() == 5, "DataFrame should have 5 row")
        self.assertTrue(res2.rows() == 5, "DataFrame should have 5 row")
        self.assertTrue(res3.rows() == 5, "DataFrame should have 5 row")
        self.assertTrue(res4.rows() == 5, "DataFrame should have 5 row")
        self.assertTrue(res5.rows() == 5, "DataFrame should have 5 row")
        self.assertTrue(res6.rows() == 5, "DataFrame should have 5 row")
        truth = self.toBeSorted.clone()
        truth.clear()
        truth.add_row(self.toBeSorted.get_row(2))
        truth.add_row(self.toBeSorted.get_row(1))
        truth.add_row(self.toBeSorted.get_row(4))
        truth.add_row(self.toBeSorted.get_row(0))
        truth.add_row(self.toBeSorted.get_row(3))
        self.assertTrue(res1.equals(truth), "DataFrames should be equal")
        self.assertTrue(res2.equals(truth), "DataFrames should be equal")
        self.assertTrue(res3.equals(truth), "DataFrames should be equal")
        self.assertTrue(res4.equals(truth), "DataFrames should be equal")
    ...
    def test_maximum_rank(self):
        ...
        self.assertTrue(res5.rows() == 1, "DataFrame should have 1 row")
        self.assertTrue(res6.rows() == 1, "DataFrame should have 1 row")
        truth = self.toBeSorted.clone().get_rows(3, 4)
        self.assertTrue(res1.equals(truth), "DataFrames should be equal")
        self.assertTrue(res2.equals(truth), "DataFrames should be equal")
        self.assertTrue(res3.equals(truth), "DataFrames should be equal")
        self.assertTrue(res4.equals(truth), "DataFrames should be equal")
    ...
    def test_round(self):
        self.df.set_column("floatCol",
                           FloatColumn(values=[10.2354, 20.2547, 30.256, 40.0, 50.515]))

        self.df.set_column("doubleCol",
                           DoubleColumn(values=[10.2354, 20.2547, 30.256, 40.0, 50.515]))
    ...
    def test_clip(self):
        ...
        truth = DefaultDataFrame(
            DataFrame.ByteColumn("byteCol", [20, 20, 30, 40, 40]),
            DataFrame.ShortColumn("shortCol", [20, 21, 31, 40, 40]),
            DataFrame.IntColumn("intCol", [20, 22, 32, 40, 40]),
            DataFrame.LongColumn("longCol", [20 ,23, 33, 40, 40]),
            DataFrame.FloatColumn("floatCol", [20.0, 20.2, 30.3, 40.0, 40.0]),
            DataFrame.DoubleColumn("doubleCol", [20.0, 21.2, 31.3, 40.0, 40.0]))
    ...
    def test_clip_with_low_unspecified(self):
        ...
        truth = DefaultDataFrame(
            DataFrame.ByteColumn("byteCol", [10, 20, 30, 40, 40]),
            DataFrame.ShortColumn("shortCol", [11, 21, 31, 40, 40]),
            DataFrame.IntColumn("intCol", [12, 22, 32, 40, 40]),
            DataFrame.LongColumn("longCol", [13 ,23, 33, 40, 40]),
            DataFrame.FloatColumn("floatCol", [10.1, 20.2, 30.3, 40.0, 40.0]),
            DataFrame.DoubleColumn("doubleCol", [11.1, 21.2, 31.3, 40.0, 40.0]))
    ...
    def test_clip_with_high_unspecified(self):
        ...
        truth = DefaultDataFrame(
            DataFrame.ByteColumn("byteCol", [20, 20, 30, 40, 50]),
            DataFrame.ShortColumn("shortCol", [20, 21, 31, 41, 51]),
            DataFrame.IntColumn("intCol", [20, 22, 32, 42, 52]),
            DataFrame.LongColumn("longCol", [20 ,23, 33, 43, 53]),
            DataFrame.FloatColumn("floatCol", [20.0, 20.2, 30.3, 40.4, 50.5]),
            DataFrame.DoubleColumn("doubleCol", [20.0, 21.2, 31.3, 41.4, 51.5]))
    ...
    def test_sort_ascend_by_byte(self):
        self.toBeSorted.sort_by("byteCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_short(self):
        self.toBeSorted.sort_by("shortCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_int(self):
        self.toBeSorted.sort_by("intCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_long(self):
        self.toBeSorted.sort_by("longCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_string(self):
        self.toBeSorted.sort_by("stringCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_char(self):
        self.toBeSorted.sort_by("charCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_float(self):
        self.toBeSorted.sort_by("floatCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_double(self):
        self.toBeSorted.sort_by("doubleCol")
        self.assertDataFrameIsSortedAscend()

    def test_sort_ascend_by_boolean(self):
        ...
        self.assertTrue(
            ...
            "Row does not match expected values at row index 2. DataFrame is not sorted correctly")
        self.assertTrue(
            self.toBeSorted.get_boolean("booleanCol", 3),
            "Row does not match expected values at row index 3. DataFrame is not sorted correctly")
        self.assertTrue(
            self.toBeSorted.get_boolean("booleanCol", 4),
            "Row does not match expected values at row index 4. DataFrame is not sorted correctly")

    def test_sort_ascend_by_binary(self):
    ...
    def test_sort_ascend_with_nans(self):
        df = DefaultDataFrame(
            DataFrame.IntColumn("A", [4, 2, 1, 5, 3]),
            DataFrame.StringColumn("B", ["4", "2", "1", "5", "3"]),
            DataFrame.FloatColumn("C", [4.0, float("NaN"), 1.0, float("NaN"), 3.0]),
            DataFrame.DoubleColumn("D", [float("NaN"), 2.0, 1.0, 5.0, float("NaN")]))

        df.sort_by("C")
        vals = df.get_column("C").as_array()
        for i, truth in enumerate([1.0, 3.0, 4.0, float("NaN"), float("NaN")]):
            if math.isnan(truth):
        ...
        for i, truth in enumerate([1.0, 2.0, 5.0, float("NaN"), float("NaN")]):
            if math.isnan(truth):
                self.assertTrue(math.isnan(vals[i]), "DataFrame is not sorted correctly")
            else:
                self.assertTrue(truth == vals[i], "DataFrame is not sorted correctly")

    def test_sort_ascend_only_nans(self):
        nan = float("NaN")
        df = DefaultDataFrame(
            DataFrame.IntColumn("A", [4, 2, 1, 5, 3]),
            DataFrame.StringColumn("B", ["4", "2", "1", "5", "3"]),
            DataFrame.FloatColumn("C", [nan, nan, nan, nan, nan]),
            DataFrame.DoubleColumn("D", [nan, nan, nan, nan, nan]))

        df.sort_by("C")
        vals = df.get_column("C").as_array()
        for val in vals:
            self.assertTrue(math.isnan(val), "DataFrame is not sorted correctly")
    ...
    def test_sort_descend_with_nans(self):
        ...
        vals = df.get_column("C").as_array()
        for i, truth in enumerate([4.0, 3.0, 1.0, float("NaN"), float("NaN")]):
            if math.isnan(truth):
                self.assertTrue(math.isnan(vals[i]), "DataFrame is not sorted correctly")
            else:
                self.assertTrue(truth == vals[i], "DataFrame is not sorted correctly")
        ...
        vals = df.get_column("D").as_array()
        for i, truth in enumerate([5.0, 2.0, 1.0, float("NaN"), float("NaN")]):
            if math.isnan(truth):
                self.assertTrue(math.isnan(vals[i]), "DataFrame is not sorted correctly")
            else:
                self.assertTrue(truth == vals[i], "DataFrame is not sorted correctly")

    def test_sort_descend_only_nans(self):
        nan = float("NaN")
        df = DefaultDataFrame(
            DataFrame.IntColumn("A", [4, 2, 1, 5, 3]),
            DataFrame.StringColumn("B", ["4", "2", "1", "5", "3"]),
    ...
    def test_space_alteration(self):
        ...
        for _ in range(5):
            self.df.add_row(
                [42, 42, 42, 42, "42", "A", 42.2, 42.2, True, bytearray.fromhex("00000080")])
